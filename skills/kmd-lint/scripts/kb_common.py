...
def validate_page_data(
    ...
    if body.strip() and body.startswith("\n"):
        warnings.append(
            "blank line between frontmatter and body — Obsidian renders "
            "it as extra space; start the body on the next line"
        )

    return errors, warnings


def _page_snippet(body: str, max_chars: int = 90) -> str:
    """First substantive body line, trimmed for an index entry."""
...
def render_index(kb: KB) -> str:
    ...
    page: wikilink, type, updated date, first substantive body line.
    """
    by_dir: dict[str, list[str]] = {}
    for page in iter_kb_pages(kb):
        rel = page.relative_to(kb.root)
        section = str(rel.parent) if str(rel.parent) != "." else "(root)"
        frontmatter, body = read_page(page)
        fm = frontmatter or {}
        page_type = fm.get("type", "?")
        updated = fm.get("updated", "?")
        snippet = _page_snippet(body)
        line = f"- [[{page.stem}]] — {page_type} · updated {updated}"
        if snippet:
            line += f" · {snippet}"
        by_dir.setdefault(section, []).append(line)

    parts = [
        "# Index",
        "",
        "Generated by `recompile_index.py` (kmd-ingest skill) — never hand-edited.",
        "",
    ]
    for section in sorted(by_dir):
        parts.append(f"## {section}")
        parts.extend(by_dir[section])
        parts.append("")

    if not by_dir:
        parts.extend(["No pages yet.", ""])
    return "\n".join(parts).rstrip() + "\n"
